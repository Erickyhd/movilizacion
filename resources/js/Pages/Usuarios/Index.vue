              ...
              <tr v-for="u in users" :key="u.id" class="hover:bg-slate-50/80 transition">
                ...
                <td class="px-6 py-4 text-right">
                  <button 
                    ...
                    class="text-xs font-semibold text-amber-600 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 px-3 py-1.5 rounded-lg inline-flex items-center space-x-1 transition"
                  >
        ...
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
          ...
          <form @submit.prevent="submitCreate" class="space-y-4">
            <div>
              ...
              <input v-model="createForm.name" type="text" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Juan Pérez" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Correo Electrónico</label>
              <input v-model="createForm.email" type="email" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="juan@empresa.com" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Contraseña Inicial</label>
              <input v-model="createForm.password" type="password" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="••••••••" />
            </div>
        ...
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
          ...
          <form @submit.prevent="submitReset" class="space-y-4">
            <div>
              ...
              <input v-model="resetForm.password" type="password" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500 outline-none" placeholder="••••••••" />
            </div>
            <div class="flex justify-end space-x-2 pt-2">
