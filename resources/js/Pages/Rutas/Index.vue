        ...
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
          ...
          <form @submit.prevent="submit" class="space-y-4">
            <div>
              ...
              <input v-model="form.origen" type="text" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Arequipa (Base Central)" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Destino</label>
              <input v-model="form.destino" type="text" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Mina Las Bambas" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Duración Estimada (Minutos)</label>
              <input v-model="form.duracion_estimada_minutos" type="number" min="1" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="240" />
            </div>
