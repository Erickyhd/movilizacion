...
const renderWeeklyChart = () => {
  ...
  // Dynamic palette colors from themeStore
  const pal = themeStore.currentPalette?.chart || {
    border: '#06b6d4',
    gradStart: 'rgba(6, 182, 212, 0.85)',
    gradEnd: 'rgba(8, 145, 178, 0.15)',
    line: '#38bdf8'
  };

  // Gradient for bars
  const gradientPax = ctx.createLinearGradient(0, 0, 0, 300);
  gradientPax.addColorStop(0, pal.gradStart);
  gradientPax.addColorStop(1, pal.gradEnd);
...
<template>
  <AppLayout>
    <div class="w-full space-y-6 pb-12">
      ...
      <div class="theme-card p-6 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-5">
        <div>
          <div class="flex items-center space-x-3 mb-1.5">
            <h2 class="text-xl font-extrabold theme-text-title tracking-tight">Centro de Control de Movilización y Transporte</h2>
          ...
          <p class="theme-text-muted text-xs font-medium">
            Monitor ejecutivo de viajes, personal acreditado, flota vehicular y estados de traslado en tiempo real.
          </p>
        </div>

        <!-- Period Selector Buttons -->
        <div class="flex flex-wrap items-center gap-2 theme-card-subtle p-1.5 rounded-xl shadow-xs">
          <button 
            @click="changePeriod('hoy')"
            :class="['px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer', activePeriod === 'hoy' ? 'theme-tab-active' : 'theme-tab-inactive']"
          >
            Hoy
          </button>
          <button 
            @click="changePeriod('semana')"
            :class="['px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer', activePeriod === 'semana' ? 'theme-tab-active' : 'theme-tab-inactive']"
          >
            Esta Semana
          </button>
          <button 
            @click="changePeriod('mes')"
            :class="['px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer', activePeriod === 'mes' ? 'theme-tab-active' : 'theme-tab-inactive']"
          >
            Este Mes
          </button>
          <button 
            @click="changePeriod('30dias')"
            :class="['px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer', activePeriod === '30dias' ? 'theme-tab-active' : 'theme-tab-inactive']"
          >
            Últimos 30 Días
          </button>
          <button 
            @click="changePeriod('todo')"
            :class="['px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer', activePeriod === 'todo' ? 'theme-tab-active' : 'theme-tab-inactive']"
          >
            Histórico Total
          </button>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <Link 
          ...
          class="theme-card theme-card-interactive p-5 group flex flex-col justify-between"
        >
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-extrabold theme-text-muted uppercase tracking-wider">Trabajadores</span>
          ...
          <div>
            <div class="text-2xl font-black theme-text-title tracking-tight">{{ stats.total_trabajadores }}</div>
            <div class="flex items-center justify-between text-xs mt-1.5 pt-2 border-t border-[var(--theme-border)]">
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-extrabold theme-text-muted uppercase tracking-wider">Manifiestos</span>
          ...
          <div>
            <div class="text-2xl font-black theme-text-title tracking-tight">{{ stats.manifiestos_periodo }}</div>
            <div class="flex items-center justify-between text-xs mt-1.5 pt-2 border-t border-[var(--theme-border)]">
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-extrabold theme-text-muted uppercase tracking-wider">Movilizados</span>
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-extrabold theme-text-muted uppercase tracking-wider">Flota Operativa</span>
          ...
          <div>
            <div class="text-2xl font-black theme-text-title tracking-tight">{{ stats.vehiculos_activos }} <span class="text-xs font-semibold text-slate-400">/ {{ stats.vehiculos_total }}</span></div>
            <div class="flex items-center justify-between text-xs mt-1.5 pt-2 border-t border-[var(--theme-border)]">
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-extrabold theme-text-muted uppercase tracking-wider">Conductores</span>
          ...
          <div>
            <div class="text-2xl font-black theme-text-title tracking-tight">{{ stats.conductores_activos }} <span class="text-xs font-semibold text-slate-400">/ {{ stats.conductores_total }}</span></div>
            <div class="flex items-center justify-between text-xs mt-1.5 pt-2 border-t border-[var(--theme-border)]">
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <span class="text-[11px] font-extrabold theme-text-muted uppercase tracking-wider">Rutas & Empresas</span>
          ...
          <div>
            <div class="text-2xl font-black theme-text-title tracking-tight">{{ stats.rutas_total }} <span class="text-xs font-semibold text-slate-400">Rutas</span></div>
            <div class="flex items-center justify-between text-xs mt-1.5 pt-2 border-t border-[var(--theme-border)]">
      ...
      <div class="theme-card p-6">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center space-x-2.5">
            <div class="w-7 h-7 rounded-lg bg-[var(--palette-600)] text-white flex items-center justify-center shadow-xs">
              <Layers class="w-4 h-4" />
            </div>
            <h3 class="font-extrabold theme-text-title text-sm tracking-tight">Pipeline de Despacho & Estados de Manifiestos</h3>
          </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3">
          ...
          <div class="p-4 rounded-xl theme-card-subtle relative overflow-hidden group">
            <div class="text-[10px] font-extrabold uppercase theme-text-muted tracking-wider">1. Registrado</div>
            <div class="text-2xl font-black theme-text-title mt-1">{{ pipeline.REGISTRADO || 0 }}</div>
            <div class="text-[10px] theme-text-muted font-semibold mt-0.5">Borrador / Preparación</div>
          </div>

          <div class="p-4 rounded-xl border border-[var(--palette-500)]/40 bg-[var(--palette-500)]/10 relative overflow-hidden group">
            <div class="text-[10px] font-extrabold uppercase text-[var(--palette-400)] tracking-wider">2. Confirmado</div>
            <div class="text-2xl font-black theme-text-title mt-1">{{ pipeline.CONFIRMADO || 0 }}</div>
            <div class="text-[10px] text-[var(--palette-400)]/80 font-semibold mt-0.5">Aprobado para Salida</div>
          </div>
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm flex flex-col justify-between">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center space-x-2.5">
              ...
              <div>
                <h3 class="font-extrabold theme-text-title text-sm tracking-tight">Movilización de Pasajeros por Día</h3>
